...
- [x] 2026-06-16 CSV leichter auffindbar: Knopf „📁 Speicherordner öffnen" (Route /api/open-folder öffnet den Ordner im Finder/Explorer) + Hinweis nennt den Dateinamen ebay-anzeigen.csv (app.py, index.html, app.js, style.css, chat_wissen.txt, Tests).
- [x] 2026-06-16 Eintrags-Verwaltung im „Übersicht"-Fenster: alle Einträge nach Status (🛠️ In Arbeit · ⏸️ Zurückgehalten · ✅ Freigegeben · 🗄️ Archiviert) mit passenden Aktionen je Zeile – Bearbeiten, Zurückhalten (speichern ohne Upload-Freigabe), Freigeben, Zurückziehen, Archivieren, Wiederherstellen, Löschen (räumt die CSV-Zeile mit auf). Sicherheitsregel: nur „Freigegeben" steht in der Upload-CSV. cases.set_case_status() + Status zurückgehalten/archiviert, ebay_csv.remove_listing(), Routen /api/draft/zurueckhalten und /api/cases/<id>/{freigeben,zurueckziehen,archivieren,wiederherstellen} (cases.py, ebay_csv.py, app.py, index.html, app.js, style.css, chat_wissen.txt, Spec, Tests) `3425317`/`b4d5a88`/`29323e5`.
- [x] 2026-06-16 „📤 Bei eBay hochladen" öffnet die eBay-Upload-Seite jetzt als echtes neues Fenster (window.open mit Fenstergröße) statt als Tab (app.js).
- [x] 2026-06-16 Roter Knopf „📤 Zum Upload" ganz unten öffnet ein Fenster, das alles rund ums Hochladen bündelt (Noch nicht hochgeladen · Hochladen & Speicherort · Stand nach Status mit „Einträge verwalten"); inline-Kachel dorthin verlegt, Zähler am Knopf; Übersicht-Fenster mit Erklärzeilen je Status und „→ Zum Upload"-Verweis (index.html, app.js, style.css) `f2de801`.
- [x] 2026-06-16 Fälle im Übersicht-Fenster tabellarisch (je Status eine Tabelle: Eintrag · Info · Aktionen, Zebra-Streifen) statt Listen; gemeinsames Suchfeld oben filtert alle Tabellen nach Autor/Titel, „Keine Treffer" je Tabelle (index.html, app.js, style.css) `23aedd5`.
- [ ] Selbst-Update Teil 2: Knopf „Helfer aktualisieren" in den Einstellungen (löst /api/update aus, Sicherheitsabfrage, Banner, Bitte um Neustart)  `→ templates/index.html, static/app.js` — wartet auf Commit der parallelen UI-Änderungen
- [x] 2026-06-16 Selbst-Update Backend: Modul updater.py (lädt GitHub-ZIP, überschreibt Code mit Sicherung + automatischem Rollback, schützt config.json/draft.json/anweisungen.txt/cases/.venv/logs, frischt Pakete auf) + Route /api/update; .update-backups/ gitignored (updater.py, app.py, .gitignore, Tests).
- [x] 2026-06-16 Löschen ist jetzt ein Soft-Delete: gelöschte Einträge werden als „gelöscht" markiert (🗑️) und bei den archivierten Einträgen einsortiert (wiederherstellbar) statt endgültig entfernt; CSV-Zeile wird weiterhin entfernt. Internes Aufräumen beim Neu-Speichern bleibt echtes Löschen (cases.py GUELTIGE_STATUS, app.py /delete + /wiederherstellen + overview, app.js, index.html, chat_wissen.txt, Tests).
